...
def buy():
    ...
    if request.method == "POST":
        ...
        # ensure correctly input shares
        if not request.form.get("shares").isdigit():
            return apology("invalid shares")
            
        quote = lookup(request.form.get("symbol"))
        db.execute("INSERT INTO transactions (user_id, symbol, price, shares) VALUES(:user_id, :symbol, :price, :shares)", user_id=session["user_id"], symbol=symbol, price=price, shares)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares > rows[0]["shares"]:
            return apology("You do not have as many shares")
